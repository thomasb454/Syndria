...
public class MoneyMechanics implements Listener {
	...
	public void onDisable() {
		int attempts = 0;
		while(bank_contents.size() > 0 && attempts <= 10) {
			attempts++;
			try {
				Thread.sleep(100);
	...
	public static ItemStack generateArrowButton(String type, int current_page, int max_pages) {
		if(type.equalsIgnoreCase("next")) {
			return ItemMechanics.signCustomItem(Material.ARROW, (short) 1, ChatColor.YELLOW.toString() + "Next Page " + ChatColor.BOLD.toString() + "->", ChatColor.GRAY.toString() + "Page " + current_page + "/" + max_pages);
		} else if(type.equalsIgnoreCase("previous")) { return ItemMechanics.signCustomItem(Material.ARROW, (short) 1, ChatColor.YELLOW.toString() + ChatColor.BOLD + "<-" + ChatColor.YELLOW.toString() + " Previous Page ", ChatColor.GRAY.toString() + "Page " + current_page + "/" + max_pages); }
		return null;
	}
	...
	public static Inventory legacyBankStringToInventory(final String p_name, String bank_string) {
		...
		for(String s : bank_content_data.split("_")) {
			...
			if(s.equalsIgnoreCase("AIR")) {
				slot_x--;
				/*
				 * try{ bank_contents.setItem(slot_x, new ItemStack(Material.AIR)); } catch(ArrayIndexOutOfBoundsException err){ err.printStackTrace();
				 * continue; }
				 */
				continue;
			}
	...
	public void updateStaticCashStack(final Player p) {
		if(!(bank_map.containsKey(p.getName()))) return;
	...
	public static void addMoneyToOfflinePlayerBank(final String p_name, int amount_to_add) {
		Hive.sql_query.add("INSERT INTO bank_database (p_name, money)" + " VALUES" + "('" + p_name + "', '" + amount_to_add + "') ON DUPLICATE KEY UPDATE money=bank_database.money+" + amount_to_add + "");

		/*
		 * PreparedStatement pst = null;
		 *
		 * try { pst = ConnectionPool.getConneciton().prepareStatement( "SELECT money FROM bank_database WHERE p_name = '" + p_name + "'");
		 *
		 * pst.execute(); ResultSet rs = pst.getResultSet(); if(!rs.next()){return;}
		 *
		 * int money = rs.getInt("money");
		 *
		 * if(money < 0){ // Negative balance? money = 0; }
		 *
		 * money += amount_to_add;
		 *
		 * pst = ConnectionPool.getConneciton().prepareStatement( "INSERT INTO bank_database (p_name, money)" + " VALUES" + "('"+ p_name + "', '"+ money
		 ...
		 * return;
		 *
		 * } catch (SQLException ex) { log.log(Level.SEVERE, ex.getMessage(), ex); return;
		 *
		 * } finally { try { if (pst != null) { pst.close(); }
		 *
	...
	public static void uploadBankDatabaseData(final String p_name, boolean remove_when_done) {
		...
		try {
			...
			/*
			 * Hive.sql_query.add("INSERT INTO bank_database (p_name, content, money, level)" + " VALUES" + "('"+ p_name + "', '"+ final_bank_content +"', '" +
			 * final_bank_net + "', '" + final_bank_level + "') ON DUPLICATE KEY UPDATE content = '" + final_bank_content + "', money='" + final_bank_net +
	...
	public ItemStack setCountMeta(ItemStack is, int amount) {
		net.minecraft.server.v1_8_R2.ItemStack nmsstack = CraftItemStack.asNMSCopy(is);
	...
	public boolean isSplitting(ItemStack i) {
		...
		if(i != null && i.hasItemMeta() && i.getItemMeta().hasLore()) {
			...
			for(String s : lore) {
				...
				if(s.contains("<SPLIT>")) { return true; }
			}
		}
		...
		/*
		 * NBTTagList description = CraftItemStack.asNMSCopy(i).getTag().getCompound("display").getList("Lore", 9); int x = 0; while(description.size() > x){
		 * if(description.get(x).toString().contains("<SPLIT>")){ return true; } x++; } return false;
		 */
	}
	...
	public static boolean isItemUnbankable(ItemStack is) {
		...
		if(!RealmMechanics.isItemTradeable(is)) {
			// Not tradeable.
			if(is.getType() == Material.BREAD || (is.getType() == Material.POTION && is.getDurability() == (short) 1)) { return true; }
		}
		return false;
	}
	...
	public int getSplitSlot(Player p) {
		...
		for(Map.Entry<Integer, ? extends ItemStack> entry : invItems.entrySet()) {
			...
			if(isSplitting(item)) { return index; }
		}
	...
	public static boolean isGemPouch(ItemStack is) {
		if(is.getType() == Material.INK_SACK && is.getDurability() == (short) 0 && is.hasItemMeta() && is.getItemMeta().hasDisplayName() && is.getItemMeta().getDisplayName().endsWith("g")) { return true; }
		return false;
	}
	...
	public int getMaxPouchCapacity(ItemStack is) {
		...
		if(i_name.contains("small")) { return 100; }
		if(i_name.contains("medium")) { return 150; }
		if(i_name.contains("large")) { return 200; }
		if(i_name.contains("gigantic")) { return 300; }
		return 0;
	}
	...
	public static int getBankNoteValue(ItemStack i) {
		...
		/*
		 * if(!(i instanceof CraftItemStack)){ return 0; } CraftItemStack css = (CraftItemStack)i; try{ try{ try{ NBTTagList description =
		 * CraftItemStack.asNMSCopy(css).getTag().getCompound("display").getList("Lore", 9); String value_string = description.get(0).toString();
		 *
		 * int value = Integer.parseInt(value_string.substring(value_string.indexOf(" ") + 1, value_string.lastIndexOf(" ")));
		 *
		 * return value;
		 *
		 * } catch(NumberFormatException e){ return 0; } } catch(IndexOutOfBoundsException e){ return 0; } } catch(NullPointerException e){ return 0; }
		 */
	}
	...
	public void upgradeBank(Player p, int new_level, boolean interval) {
		...
		if(new_level == 6 || new_level == 12) {
			...
			Inventory new_page = Bukkit.createInventory(null, 9 + 9, "Bank Chest (" + current_page + "/" + total_pages + ")"); // 9 slots, extra 9 at bottom for
																																// previous button.
			...
			for(ItemStack is : bank_contents.get(p.getName()).get(last_page_index)) {
				if(is != null && !(is.getType() == Material.AIR) && is.getType() != Material.EMERALD && !isDivider(is) && !isPreviousArrow(is) && !isNextArrow(is)) {
					i.setItem(i.firstEmpty(), is);
				}
			}
	...
	public void onPlayerChatEvent(AsyncPlayerChatEvent e) {
		...
		if(bank_upgrade_codes.containsKey(p)) {
			...
			if(e.getMessage().contains(auth_code)) {
				...
				/*
				 * if(new_tier > 5){ p.sendMessage(ChatColor.RED + "You cannot upgrade your bank; already at highest available tier.");
				 * bank_upgrade_codes.remove(p); return; }
				 */
		...
		if(withdraw_map.containsKey(p)) {
			...
			if(withdraw_type.get(p).equalsIgnoreCase("raw")) {
				...
				p.sendMessage(ChatColor.GREEN + "" + ChatColor.BOLD + "New Balance: " + ChatColor.GREEN + (net_worth - withdraw_amount) + " GEM(s)");
				// p.sendMessage(ChatColor.GRAY + "You have converted " + withdraw_amount + " GEM(s) from your bank account into a " + ChatColor.BOLD.toString()
				// + "GEM NOTE.");
				p.sendMessage(ChatColor.GRAY + "Banker: " + ChatColor.WHITE + "Here are your Gems, thank you for your business!");
			}
	...
	public void onPlayerInteract(PlayerInteractEvent e) {
		...
		if(e.hasItem() && isGemPouch(e.getItem())) {
			...
			pl.sendMessage(ChatColor.YELLOW + "To " + ChatColor.BOLD + "DEPOSIT" + ChatColor.YELLOW + " GEM(s), drag them onto the pouch in your inventory. To " + ChatColor.BOLD + "WITHDRAW" + ChatColor.YELLOW + " GEM(s), " + ChatColor.UNDERLINE + "right click" + ChatColor.YELLOW + " the pouch in your inventory.");
		...
		if(i.getType() == Material.PAPER && (e.getAction() == Action.LEFT_CLICK_AIR || e.getAction() == Action.LEFT_CLICK_BLOCK)) {
			...
			if(i.getAmount() > 1) { return; // Do not split stacked notes.
			}
				...
				if(!(b.getRelative(f, r) == null)) {
					...
					for(int x = -r; x <= r; x++) {
						for(int y = -r; y <= r; y++) {
							...
							if(a.getType() == Material.ENDER_CHEST) return true;
						}
					}
				}
			}
	...
	public boolean isStaticGem(ItemStack is) {
		if(is != null && is.getType() == Material.EMERALD && is.hasItemMeta() && is.getItemMeta().hasDisplayName() && is.getItemMeta().getDisplayName().endsWith(ChatColor.BOLD.toString() + " GEM(s)")) { return true; }
		return false;
	}
	...
	public void onInventoryCloseEvent(InventoryCloseEvent e) throws IOException {
		...
		if(e.getInventory().getName().startsWith("Bank Chest")) {
			...
			if(junk_count >= 100) {
				AchievementMechanics.addAchievement(p.getName(), "I might need it later!");
			}

			if(weapon_count >= 10) {
				AchievementMechanics.addAchievement(p.getName(), "Personal Arsenal");
			}
			...
			if(bank_map.containsKey(p.getName())) {
				...
				if(bank_net >= 100) {
					AchievementMechanics.addAchievement(p.getName(), "Acquire Currency I");
					if(bank_net >= 1000) {
						AchievementMechanics.addAchievement(p.getName(), "Acquire Currency II");
						if(bank_net >= 5000) {
							AchievementMechanics.addAchievement(p.getName(), "Acquire Currency III");
							if(bank_net >= 10000) {
								AchievementMechanics.addAchievement(p.getName(), "Acquire Currency IV");
								if(bank_net >= 50000) {
									AchievementMechanics.addAchievement(p.getName(), "Acquire Currency V");
									if(bank_net >= 100000) {
										AchievementMechanics.addAchievement(p.getName(), "Acquire Currency VI");
										if(bank_net >= 500000) {
											AchievementMechanics.addAchievement(p.getName(), "Acquire Currency VII");
											if(bank_net >= 1000000) {
												AchievementMechanics.addAchievement(p.getName(), "Acquire Currency IX");
											}
										}
									}
								}
	...
	/*
	 * @EventHandler(priority = EventPriority.HIGH, ignoreCancelled = true) public void onGemPouchClickEvent(InventoryClickEvent e){ Player pl =
	 * (Player)e.getWhoClicked(); if(e.getCurrentItem() != null || e.getCursor() != null){ ItemStack current = null; ItemStack cursor = null;
	 * if(e.getCurrentItem() != null){ current = e.getCurrentItem(); if(isGemPouch(current)){ if(current.getAmount() > 1){ // TODO: Unstack
	 * unstackAllPouches(pl); return; // Once is enough. } } } if(e.getCursor() != null){ cursor = e.getCursor(); if(isGemPouch(cursor)){ if(cursor.getAmount()
	 * > 1){ // TODO: Unstack unstackAllPouches(pl); return; } } } } }
	 */
	...
	public void onGemPouchWithdraw(InventoryClickEvent e) {
		Player pl = (Player) e.getWhoClicked();
		if(e.isLeftClick() || e.isShiftClick() || (e.getCursor() != null && e.getCursor().getType() != Material.AIR) || e.getCurrentItem() == null || !(isGemPouch(e.getCurrentItem()))) { return; // We don't care. Right click only.
		}

		if(!pl.getOpenInventory().getTopInventory().getName().equalsIgnoreCase("container.crafting")) { return; }

		ItemStack pouch = e.getCurrentItem();

		if(pouch.getAmount() > 1) { return; }
	...
	public void onGemPouchDeposit(InventoryClickEvent e) {
		Player pl = (Player) e.getWhoClicked();
		if(e.isRightClick() || e.getCursor() == null || e.getCurrentItem() == null || e.getCursor().getType() != Material.EMERALD || !(isGemPouch(e.getCurrentItem()))) { return; // We don't care.
		}
		...
		if(max_capacity == current_value) { return; // We ain't gonna do anything.
		}
	...
	public void onPlayerClickBank(InventoryClickEvent e) {
		...
		if(e.getInventory().getName().startsWith("Bank Chest")) {
			...
			if(isPreviousArrow(clicked)) {
				...
				Main.plugin.getServer().getScheduler().scheduleSyncDelayedTask(Main.plugin, new Runnable() {
					public void run() {
						p.openInventory(bank_contents.get(p.getName()).get(current_page - 2)); // Subtract 1 for index, 1 for previous page. Page 2 - > Page 0
																								// .get(0) (which is actually page 1)
						p.playSound(p.getLocation(), Sound.BAT_TAKEOFF, 1F, 1.2F);
					}
				}, 2L);
				return;
	...
	public void onPlayerClickInventory(InventoryClickEvent event) {
		...
		if(event.getCurrentItem() != null && isDivider(event.getCurrentItem())) {
		...
		if(event.getInventory().getName().startsWith("Bank Chest") && (((event.getCursor() != null && event.getCursor().getType() == Material.EMERALD) || (event.getCursor() != null && event.getCursor().getType() == Material.PAPER) || (event.getCursor() != null && isGemPouch(event.getCursor()))) || (event.isShiftClick() && ((event.getCurrentItem() != null && event.getCurrentItem().getType() == Material.EMERALD) || (event.getCurrentItem() != null && event.getCurrentItem().getType() == Material.PAPER) || (event.getCurrentItem() != null && isGemPouch(event.getCurrentItem())))))) {
			// ((((event.getCursor() != null && event.getCursor().getType() == Material.EMERALD) || (event.getCurrentItem() != null &&
			// event.getCurrentItem().getType() == Material.EMERALD)))
			// || (((event.getCursor() != null && event.getCursor().getType() == Material.PAPER) || (event.getCurrentItem() != null &&
			...
			if(amount_to_deposit > 0) {
				...
				// p.sendMessage(ChatColor.GREEN + "Deposited " + amount_to_deposit + " GEM(s) into your bank chest. Your NEW balance is " + ChatColor.BOLD +
				// (amount_to_deposit + cur_val) + " GEM(s)");
				// p.sendMessage(ChatColor.GRAY + "To DEPOSIT gems, " + ChatColor.UNDERLINE + "RIGHT CLICK" + ChatColor.GRAY +
				// " the cauldron with gems in your character's hand. To WITHDRAW gems, "
