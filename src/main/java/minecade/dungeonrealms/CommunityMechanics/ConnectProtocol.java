			...
			while ((inputLine = in.readLine()) != null) {
				...
				if (inputLine.startsWith("@ban@")) { // @ban@notch:reason
					...
					if (Bukkit.getPlayer(p_name_2ban) != null) {
						Hive.to_kick.put(
								p_name_2ban,
								ChatColor.RED + "Your account has been " + ChatColor.UNDERLINE + "TEMPORARILY" + ChatColor.RED
								+ " locked due to suspisious activity." + "\n" + ChatColor.RED + "Reason: " + ChatColor.GRAY + reason + "\n\n"
				...
				if (inputLine.startsWith("@mute@")) { // @mute@player_mutie/player_muted:unmute_time
					...
					if (Bukkit.getPlayer(p_name_muted) != null) {
						...
						muted.sendMessage("");
						muted.sendMessage(ChatColor.RED + "You have been " + ChatColor.BOLD + "GLOBALLY MUTED" + ChatColor.RED + " by " + ChatColor.BOLD
								+ p_name_mutie + ChatColor.RED + " for " + unmute_time + " minute(s).");
						muted.sendMessage("");
					}
				}
				...
				if (inputLine.startsWith("@money@")) {
					...
					if (Bukkit.getPlayer(owner_name) != null && Bukkit.getPlayer(owner_name).isOnline()) {
						...
						p_owner_name.sendMessage(ChatColor.GREEN + "SOLD " + amount + "x '" + ChatColor.WHITE + i_name + ChatColor.GREEN + "' for "
					...
					for (String s : GuildMechanics.getGuildMembers(g_name)) {
						if (Bukkit.getPlayer(ChatColor.stripColor(s)) != null) {
							...
							pty_mem.sendMessage(ChatColor.DARK_AQUA + "<" + ChatColor.BOLD + GuildMechanics.guild_handle_map.get(g_name) + ChatColor.DARK_AQUA
									+ "> " + ChatColor.DARK_AQUA.toString() + p_name + ChatColor.GRAY.toString() + " has " + ChatColor.UNDERLINE + "joined"
				...
				if (inputLine.startsWith("[gquit]")) {
					...
					for (String s : GuildMechanics.getOnlineGuildMembers(g_name)) {
						if (Bukkit.getPlayer(ChatColor.stripColor(s)) != null) {
							Player pl = Bukkit.getPlayer(ChatColor.stripColor(s));
							pl.sendMessage(ChatColor.DARK_AQUA + "<" + ChatColor.BOLD + GuildMechanics.guild_handle_map.get(g_name) + ChatColor.DARK_AQUA
									+ "> " + ChatColor.DARK_AQUA + p_name + ChatColor.GRAY + " has " + ChatColor.UNDERLINE + "left" + ChatColor.GRAY
									+ " the guild.");
						}
					}
				}
				...
				if (inputLine.startsWith("[gkick]")) {
					...
					for (String s : GuildMechanics.getOnlineGuildMembers(g_name)) {
						if (Bukkit.getPlayer(ChatColor.stripColor(s)) != null) {
							Player pl = Bukkit.getPlayer(ChatColor.stripColor(s));
							pl.sendMessage(ChatColor.DARK_AQUA + "<" + ChatColor.BOLD + GuildMechanics.guild_handle_map.get(g_name) + ChatColor.DARK_AQUA
									+ "> " + ChatColor.DARK_AQUA + p_name + " has been " + ChatColor.UNDERLINE + "kicked" + ChatColor.DARK_AQUA + " by "
									+ p_kicker + ".");
						}
					}
				}
				...
				if (inputLine.startsWith("[quit]")) {
					...
					for (String g_member : GuildMechanics.getGuildMembers(g_name)) {
						...
						pl_g_member.sendMessage(ChatColor.DARK_AQUA + "<" + ChatColor.BOLD + GuildMechanics.guild_handle_map.get(g_name) + ChatColor.DARK_AQUA
								+ "> " + ChatColor.GRAY + p_name + " has logged out of " + server_name + ".");
					}
				...
				if (inputLine.startsWith("[join]")) {
					...
					for (String g_member : GuildMechanics.getGuildMembers(g_name)) {
						...
						pl_g_member.sendMessage(ChatColor.DARK_AQUA + "<" + ChatColor.BOLD + GuildMechanics.guild_handle_map.get(g_name) + ChatColor.DARK_AQUA
								+ "> " + ChatColor.GRAY + p_name + " has joined " + server_name + ".");
				...
				if (inputLine.startsWith("&")) {
					if (inputLine.substring(inputLine.indexOf("&") + 1, inputLine.indexOf("/")).equalsIgnoreCase("staffchat")) {
						...
						String sender_name = inputLine.substring(inputLine.indexOf("*") + 1, inputLine.indexOf(":"));
						...
						for (Player pl : Bukkit.getOnlinePlayers()) {
							if (PermissionMechanics.isStaff(pl)) {
								...
								String prefix = ChatMechanics.getPlayerPrefix(sender_name);
								String message = raw_message;

								if (ChatMechanics.hasAdultFilter(pl.getName())) message = ChatMechanics.censorMessage(message);
				...
				if (inputLine.startsWith("^")) { // Sending message.
					...
					if (Bukkit.getPlayer(p_to_name) != null) {
						...
						if (!PermissionMechanics.getRank(p_from_name).equalsIgnoreCase("gm")
								&& !PermissionMechanics.getRank(p_from_name).equalsIgnoreCase("pmod")
								&& PlayerManager.getPlayerModel(p_to).getToggleList().contains("tells")
								&& !(CommunityMechanics.isPlayerOnBuddyList(p_to, p_from_name))) {
						...
						if (!PermissionMechanics.getRank(p_from_name).equalsIgnoreCase("gm")
								&& !PermissionMechanics.getRank(p_from_name).equalsIgnoreCase("pmod")
								&& CommunityMechanics.isPlayerOnIgnoreList(p_to, p_from_name)) {
						...
						String result = "#" + p_from_name + "/" + to_prefix + to_c.toString() + p_to_name + "=" + "online,"
								+ Bukkit.getMotd().substring(0, Bukkit.getMotd().length()) + ":" + message;
						sendResultCrossServer(server_ip, result, server_num);
