...
public class HealthMechanics implements Listener {
	...
	public static void setOverheadHP(Player pl, int hp) {
		...
		if(!pl.hasMetadata("NPC") && !pl.getPlayerListName().equalsIgnoreCase("")) {
			...
			if(lvl != null){
				...
				BossbarAPI.setMessage(pl, levelData + dash + ChatColor.LIGHT_PURPLE.toString() + ChatColor.BOLD.toString() + "HP " + ChatColor.LIGHT_PURPLE + getPlayerHP(pl.getName()) + ChatColor.BOLD.toString() + " / " + ChatColor.LIGHT_PURPLE.toString() + getMaxHealthValue(pl.getName()) + dash + xpData, (float) (health_percent * 100F));
			}else{
				BossbarAPI.setMessage(pl, ChatColor.LIGHT_PURPLE.toString() + ChatColor.BOLD.toString() + "HP " + ChatColor.LIGHT_PURPLE + getPlayerHP(pl.getName()) + ChatColor.BOLD.toString() + " / " + ChatColor.LIGHT_PURPLE.toString() + getMaxHealthValue(pl.getName()), (float) (health_percent * 100F));
			}
	...
	public static int generateMaxHP(Entity e) {
		...
		net.minecraft.server.v1_8_R2.ItemStack[] armor_contents = ent.getEquipment();
		double total_health = 0;
		for(net.minecraft.server.v1_8_R2.ItemStack i : armor_contents) {
